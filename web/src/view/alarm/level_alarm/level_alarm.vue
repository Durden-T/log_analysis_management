    ...
    >
      ...
      <el-table-column label="">
        <template slot-scope="scope">
          <el-button
            class="table-button"
            @click="updateLevelAlarmStrategy(scope.row)"
            size="small"
            type="primary"
            icon="el-icon-edit"
            >变更</el-button
...
<script>
import {
  createLevelAlarmStrategy,
  deleteLevelAlarmStrategy,
  deleteLevelAlarmStrategyByIds,
  updateLevelAlarmStrategy,
  findLevelAlarmStrategy,
  getLevelAlarmStrategyList,
} from "@/api/level_alarm"; //  此处请自行替换地址
import { formatTimeToStr } from "@/utils/date";
import infoList from "@/mixins/infoList";
export default {
  name: "LevelAlarmStrategy",
  mixins: [infoList],
  data() {
    return {
      listApi: getLevelAlarmStrategyList,
  ...
  methods: {
    ...
    deleteRow(row) {
      this.$confirm("确定要删除吗?", "提示", {
        ...
        this.deleteLevelAlarmStrategy(row);
      });
    },
    async onDelete() {
      const ids = [];
      ...
      const res = await deleteLevelAlarmStrategyByIds({
        ID: ids,
    ...
    async updateLevelAlarmStrategy(row) {
      const res = await findLevelAlarmStrategy({ ID: row.ID, app: row.app });
      this.type = "update";
      if (res.code == 0) {
    ...
    async deleteLevelAlarmStrategy(row) {
      const res = await deleteLevelAlarmStrategy({ ID: row.ID, app: row.app });
      if (res.code == 0) {
        this.$message({
          type: "success",
          message: "删除成功",
    ...
    async enterDialog() {
      ...
      switch (this.type) {
        case "create":
          res = await createLevelAlarmStrategy(this.formData);
          break;
        case "update":
          res = await updateLevelAlarmStrategy(this.formData);
          break;
        default:
          res = await createLevelAlarmStrategy(this.formData);
          break;
